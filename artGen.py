...
def training(n_epochs, file, chunk_count=10):
    ...
    start = time.time()
    all_losses = []
    loss_avg = 0    # moyenne de loss sur tout les train
    best_loss = 100
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if epoch % eval_every == 0:
            print()
            print(f"Evaluation à l'epoch {epoch}:")
            print(evaluate(decoder, prime_str='Wh'))
            print()
...
def evaluating(decoder, length):
    ...
    try:
        while True:
            print('Enter a starting two or three characters')
            input1 = input()
            print()
            if len(input1) > 0:
